...
public class Database extends SQLiteOpenHelper {
    ...
    public void checkTable(String date) {
        String create = "CREATE TABLE IF NOT EXISTS `" + date +
                "` (`ID` integer, `Task` text, `From` text, `To` text, `Color` text, `RoomNumber` text, `Teacher` text);";
    ...
    public void addTask(Task t, String date) {
        checkTable(date);
        SQLiteDatabase db = this.getWritableDatabase();
        String insert = "INSERT INTO `" + date + "` (`ID`, `Task`, `From`, `To`, `Color`, `RoomNumber`, `Teacher`) VALUES " +
                "('" + t.getID() + "', '" + t.getTask() + "', '" + t.getFromToString() + "', '" + t.getToToString() +
                "', '" + t.getColor() + "', '" + t.getRoomNumber() + "', '" + t.getTeacher() + "');";
        db.execSQL(insert);
    }
    ...
    public ArrayList<Task> getAllTasks(String date) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                t.setRoomNumber(cursor.getString(5)); // Устанавливаем номер кабинета
                t.setTeacher(cursor.getString(6)); // Устанавливаем преподавателя
                tasks.add(t);
    ...
    public void updateTask(Task t, String date) {
        String update = "UPDATE `" + date + "` SET `Task` = '" + t.getTask() + "', `From` = '" +
                ...
                "', `RoomNumber` = '" + t.getRoomNumber() + "', `Teacher` = '" + t.getTeacher() + "' WHERE `ID` = " + t.getID() + ";";
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL(update);
    }
